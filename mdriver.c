...
static double eval_libc_util(trace_t *trace, int tracenum, range_t **ranges);
...
int main(int argc, char **argv)
{
    ...
    int ignore_mm_malloc = 0;  /* If set, don't test the student mm (set by -L) */
     ...
     */
    while ((c = getopt(argc, argv, "f:t:hvVgalL")) != EOF) {
        switch (c) {
    ...
    if (run_libc) {
        ...
        for (i=0; i < num_tracefiles; i++) {
            ...
            if (libc_stats[i].valid) {
                if (verbose > 1) 
                    printf("efficiency, ");
                libc_stats[i].util = eval_libc_util(trace, i, &ranges);
    ...
    /*
     * if ignore_mm_malloc isn't set, run and evaluate the student's mm package
     */
    if(ignore_mm_malloc) {
        exit(0);
    }
...
/* 
 * eval_libc_util - Evaluate the space utilization of the libc malloc
 *   package The idea is to remember the high water mark "hwm" of the
 *   heap for an optimal allocator, i.e., no gaps and no internal
 *   fragmentation.  Utilization is the ratio hwm/heapsize, where
 *   heapsize is the size of the heap in bytes after running the
 *   system malloc package on the trace.  This assumes that malloc
 *   puts things into the heap contiguously.  This appears to be the
 *   case on linux, but not on OS X.  It also assumes that the system
 *   starts allocating at the bottom of the heap.
 */
static double eval_libc_util(trace_t *trace, int tracenum, range_t **ranges)
{   
    int i;
    ...
    unsigned min_address = 0xffffffff;
    unsigned max_address = 0;  // won't take into account any footer on the last alloc, but
                          // that shouldn't matter
    
#ifdef __linux__
    // will actually ask the glibc malloc to tell us how much memory it's using
    // see http://www.gnu.org/software/libtool/manual/libc/Statistics-of-Malloc.html
    struct mallinfo malloc_data;
    unsigned max_used_mem = 0;
#endif
    ...
    for (i = 0;  i < trace->num_ops;  i++) {
        ...
        case ALLOC: /* alloc */
            ...
            if ((p = malloc(size)) == NULL) 
                app_error("malloc failed in eval_libc_util");

            min_address = ((unsigned)p < min_address)?(unsigned)p : min_address;
            max_address = ((unsigned)(p+size) > max_address)?(unsigned)(p+size) : max_address;
                
#ifdef __linux__
	    malloc_data = mallinfo();
	    max_used_mem = (malloc_data.arena > max_used_mem)? malloc_data.arena: max_used_mem;
#endif
            /* Remember region and size */
            trace->blocks[index] = p;
            trace->block_sizes[index] = size;
        ...
        case REALLOC: /* realloc */
            ...
            oldp = trace->blocks[index];
            if ((newp = realloc(oldp,newsize)) == NULL)
                app_error("realloc failed in eval_libc_util");


            min_address = ((unsigned)newp < min_address)?(unsigned)newp : min_address;
            max_address = ((unsigned)(newp+newsize) > max_address)?(unsigned)(newp+newsize) : max_address;
        ...
        default:
            app_error("Nonexistent request type in eval_libc_util");

        }
    }
    //printf("\neval_libc_util(): min_address=0x%x  max_address=0x%x\n", min_address, max_address);

    int usedmem = max_address - min_address;
#ifdef __linux__
//    printf("LINUX!  my guess for total used mem: %d\n", usedmem);
    malloc_data = mallinfo();
    usedmem = max_used_mem;
//    printf("LINUX!  malloc's version of the heap high water mark: %d\n", usedmem);

#endif
    return ((double)max_total_size / (double)usedmem);
}
...
static void usage(void) 
{
    ...
    fprintf(stderr, "\t-L         Run libc malloc ONLY.\n");
